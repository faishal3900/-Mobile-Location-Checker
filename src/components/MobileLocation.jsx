...
const MobileLocation = () => {
    ...
    const submitLocation = async (e) => {
        ...
        try {
            const res = await fetch(
                `https://apilayer.net/api/validate?access_key=${API_KEY}&number=${phoneNumber}&country_code=IN`
            );
            const data = await res.json();
            setLocation(data);
        } catch (error) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-600 to-pink-500 p-4">
            <div className="bg-white  bg-opacity-20 backdrop-blur-lg rounded-xl shadow-2xl p-8 w-full max-w-md borde">
                <h2 className="text-2xl font-semibold text-center mb-6">📍 Mobile SIM Location Checker</h2>
                ...
                <form className="flex flex-col gap-4" onSubmit={submitLocation}>
                    ...
                    <button
                        type="submit"
                        className=" bg-gradient-to-br from-pink-500  hover:bg-yellow-500 text-white font-bold py-2 rounded transition duration-300 cursor-pointer "
                    >
                        🔍 Check Location
                    </button>
                </form>
